...
* `EJECUTAR`: Habilita la acción del botón **"Exportar Asignación Seleccionada"** (`_on_exportar_lote_seleccionado`), así como las acciones internas del diálogo de detalle: **"Generar Excel"** (`_on_generate_excel`), **"Generar PDF"** (`_on_generate_pdf`) para la expedición formal de documentos individuales por referencia, y **"Unificar"** (`_on_generate_unified_pdf`) para la consolidación de todas las referencias seleccionadas en un único archivo PDF unificado. En caso de ausencia de permiso, el sistema deniega el acceso e informa mediante `QMessageBox.warning`.
