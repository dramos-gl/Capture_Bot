...
CREATE TABLE sar_archivo.asignacion (
    asignacion_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
SELECT setval(COALESCE(pg_get_serial_sequence('sar_archivo.asignacion', 'asignacion_id'), 'sar_archivo.asignacion_asignacion_id_seq'), COALESCE((SELECT MAX(asignacion_id) FROM sar_archivo.asignacion), 0) + 1, false);
